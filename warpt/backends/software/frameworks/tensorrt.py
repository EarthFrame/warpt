"""NVIDIA TensorRT framework detection."""

from warpt.backends.software.frameworks.base import FrameworkDetector
from warpt.models.list_models import FrameworkInfo


class TensorRTDetector(FrameworkDetector):
    """Detector for NVIDIA TensorRT installation."""

    @property
    def framework_name(self) -> str:
        """Return the canonical name of the framework."""
        return "tensorrt"

    def detect(self) -> FrameworkInfo | None:
        """Detect TensorRT installation and gather version information.

        Returns
        -------
            FrameworkInfo with version if installed, None otherwise.
        """
        tensorrt = self._safe_import("tensorrt")
        if tensorrt is None:
            return None

        # Get version
        try:
            version = tensorrt.__version__  # type: ignore[attr-defined]
        except AttributeError:
            version = "unknown"

        # TensorRT is GPU-only and requires CUDA
        return FrameworkInfo(
            installed=True,
            version=version,
            cuda_support=True,
        )
