"""Storage random read test for measuring random I/O performance (IOPS).

This test measures random read performance with small block sizes:
- Creates test file during setup
- Reads 4KB blocks at random offsets
...
class StorageRandomReadTest(StressTest):
    """Random read test for measuring storage IOPS.

    Tests random read performance by reading small blocks at random offsets.
    This simulates real-world usage patterns like OS boot, app launches,
    and database operations.

    Measures:
    - Random read IOPS (I/O Operations Per Second)
    - Average latency per operation
    - Sustained random read performance
    ...
    def __init__(
        ...
        """Initialize storage random read test.

        Args:
            file_size_mb: Size of test file in megabytes.
            block_size_kb: Block size for reads in kilobytes (typically 4KB).
    ...
    def get_name(self) -> str:
        ...
        return "storage_random_read"

    def get_pretty_name(self) -> str:
        """Return human-readable test name."""
        return "Storage Random Read Test"

    def get_description(self) -> str:
        """Return one-line description."""
        return "Measures random read IOPS with 4KB blocks"
    ...
    def setup(self) -> None:
        ...
        self._test_file = os.path.join(test_dir, "warpt_random_read_test.dat")
    ...
    def execute_test(self, duration: int, iterations: int) -> dict[Any, Any]:
        ...
        self.logger.info(f"Running random read test for {duration}s...")
        ...
        self.logger.info(f"Completed {total_operations} random reads in {elapsed:.2f}s")
